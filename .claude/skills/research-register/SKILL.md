...
Every production module, including implementation helpers and protocol implementations, has substantive module documentation grounded in its current behavior. Do not use `@moduledoc false` in `lib/`. Keep internal status explicit in the prose. Every test and test-support module uses `@moduledoc false` followed by exactly one blank line. Executable module examples use `iex>` prompts and are exercised by a corresponding `doctest` declaration; configuration or external-resource sketches are labeled as such.
